...
sub build_example2($s) is export {
  my $range = GtkSheetRange.new;
  my $color = GTK::Compat::RGBA.new;

  $s.name = 'example2';
  $s.set_autoscroll(True);
  $s.set_selection_mode(GTK_SELECTION_SINGLE);
  $range = GtkSheetRange.new(0, 0, 2, $s.maxcol);
  $range.set_editable($range, False);
  ...
  #
  # $s.set_cell.tap({ parse_numbers( $s ) });

  # $s.set_row_hreight(12, 60);
  ...
  #   $s.range_set_border_color($, $color);
  #   $s.range_set_border(
  #     $, $b, $_ - 3, CAIRO_LINE_CAP_BUTT, CAIRO_LINE_JOIN_MITER
  #   );
  # }
}
